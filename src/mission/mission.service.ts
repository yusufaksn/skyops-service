...
import { MissionStrategy } from './strategies/mission-status.strategy'
...
async create(dto: CreateMissionDto): Promise<Mission> {
  ...
  try {
    ...
    // Artık throw fırlatılsa bile burası güvenle çalışır ve bağlantıyı havuza bırakır
  ...
  const strategy = this.strategies.find((s) =>
    s.supports(dto.status),
  );

  if (!strategy) {
    throw new BadRequestException(
      `Invalid target status: ${dto.status}`,
    );
  }

  

  const result = await strategy.execute(missionId, dto);
